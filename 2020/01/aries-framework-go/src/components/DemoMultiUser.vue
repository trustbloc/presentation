    ...
    <div class="eg-transition" :enter='enter' :leave='leave'>
        <div class="eg-slide" v-if="active">
            <div class="eg-slide-content">
                ...
                <div v-if="step < 5">
                    <p v-if="invitation != null && step > 1">{{ invitation.label }} endpoint: <b>{{ invitation[0].serviceEndpoint }}</b></p>
                </div>
        ...
        methods: {
            getBotInvitation: function() {
                this.ariesBotAgentClient.didexchange.CreateInvitation({}).then(res => {
                    this.invitation.push(res.data.invitation);
                })
            },
            connectToBot: async function() {
                if (this.invitation.length === 0) {
                    ...
                    for (var i = 0; i < this.invitation.length; i++) {
                        ...
                        try {
                            let res = await this.ariesHumanAgentClient.didexchange.ReceiveInvitation(this.invitation[i])
            ...
            botConnStatus: async function() {
                for (const connectionID of this.connectionID) {
                    try {
                        let res = await this.ariesHumanAgentClient.didexchange.QueryConnectionByID(connectionID)
